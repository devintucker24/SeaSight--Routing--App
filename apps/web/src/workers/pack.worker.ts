...
self.onmessage = async (event: MessageEvent) => {
  ...
  try {
    if (type === 'LOAD_PACK') {
      ...
      self.postMessage({ type: 'PACK_LOADED', payload: { success: true, packData: currentPack } });
      ...
      if (environmentSampler) {
        ...
        self.postMessage({ type: 'ENVIRONMENT_SAMPLE', payload: sample });
      } else {
        self.postMessage({ type: 'ERROR', payload: 'Environment sampler not initialized.' });
      }
    }
  } catch (error: any) {
