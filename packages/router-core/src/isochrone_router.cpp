...
IsochroneRouter::Result IsochroneRouter::solve(const Request& request, const EnvironmentSampler& sampler) const {
    ...
    start_state.hazard_flags = HazardFlags::NONE;
...
#endif
    int rejection_logs = 0;
    const int max_rejection_logs = 100;
    auto logRejection = [&](const char* reason, const GeoPoint& location, double depth, double wave) {
#ifdef __EMSCRIPTEN__
        if (rejection_logs >= max_rejection_logs) {
            return;
        }
        emscripten::val payload = emscripten::val::object();
        payload.set("reason", std::string(reason));
        payload.set("lat", location.lat);
        payload.set("lon", location.lon);
        payload.set("depth", depth);
        payload.set("wave", wave);
        console.call<void>("warn", std::string("[Isochrone] Leg rejected"), payload);
        ++rejection_logs;
#else
        (void)reason;
        (void)location;
    ...
    auto logHazard = [&](const char* reason, const GeoPoint& location, double depth, double wave) {
#ifdef __EMSCRIPTEN__
        emscripten::val payload = emscripten::val::object();
        payload.set("reason", std::string(reason));
        payload.set("lat", location.lat);
        ...
        payload.set("depth", depth);
        payload.set("wave", wave);
        console.call<void>("info", std::string("[Isochrone] Hazard recorded"), payload);
#else
        (void)reason;
        (void)location;
        (void)depth;
        ...
        for (int idx : frontier) {
            ...
            for (int h = 0; h < settings.heading_count; ++h) {
                ...
                if (env_dst.depth_m + EPS < min_depth) {
                    logRejection("destinationDepth", destination, env_dst.depth_m, env_dst.wave_height_m);
                    continue;
                }
                ...
                if (wave_hazard) {
                    logHazard("waveLimit", destination, env_dst.depth_m, env_dst.wave_height_m);
                }

                // Sample intermediate points along the geodesic arc to ensure no land crossings.
                int intermediate_samples = std::clamp(static_cast<int>(distance_nm / 20.0), 2, 12);
                bool intersects_land = false;
                for (int s = 1; s < intermediate_samples; ++s) {
                    double fraction = static_cast<double>(s) / intermediate_samples;
                    ...
                    if (env_mid.depth_m + EPS < min_depth) {
                        logRejection("midpointDepth", midpoint, env_mid.depth_m, env_mid.wave_height_m);
                        intersects_land = true;
                        break;
                    }

                    peak_wave_height = std::max(peak_wave_height, env_mid.wave_height_m);
                    if (peak_wave_height > request.ship.max_wave_height_m) {
                        logHazard("waveLimit", midpoint, env_mid.depth_m, peak_wave_height);
                        wave_hazard = true;
                        break;
                    }
                }
                ...
                candidate.hazard_flags = current.hazard_flags | (wave_hazard ? IsochroneRouter::HazardFlags::HIGH_WAVE : IsochroneRouter::HazardFlags::NONE);
    ...
    diagnostics.hazard_flags = final_state.hazard_flags;
